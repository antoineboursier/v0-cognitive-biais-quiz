...
export function PricingGridQuestion({ options, onSelect, disabled }: PricingGridQuestionProps) {
  return (
    <div className="w-full max-w-4xl mx-auto">
      <p className="text-primary font-mono text-sm mb-4 text-center">
        ...
        {options.map((option, index) => (
          <motion.button
            ...
            className={`relative p-6 rounded-xl border-2 transition-all duration-300 text-left ${
              option.highlighted ? "border-primary bg-primary/10" : "border-border bg-secondary/50"
            } ${!disabled ? "hover:scale-105 hover:border-primary cursor-pointer" : "cursor-default"}`}
          ...
          >
            ...
            <p className="text-3xl font-bold text-primary mb-4">{option.price}</p>
            ...
            <div
              className={`mt-6 py-2 px-4 rounded-lg text-center font-semibold ${
                option.highlighted ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
              }`}
            >
              Choisir {option.name}
            </div>
